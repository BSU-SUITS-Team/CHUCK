...
def format_prompt_with_context(user_prompt: str, context: dict[str, Any]) -> str:
    ...
    return (
        ...
        "requests as coming from that EVA; the user is that EVA astronaut. "
        "For biometric answers, mention only that user's EVA unless the user "
        "explicitly asks about another EVA, both EVAs, or a comparison. "
        "User prompts may be audio transcripts; if a word seems "
        "wrong but a similar-sounding mission or procedure word makes the "
        "query coherent, use your best judgment about the intended word. "
        "For EVA mission-state and procedure questions, the fetched "
        ...
        "procedure details from names. Do not call a tool when the needed "
        "information is already available in the current prompt, fetched "
        "context, previous tool results, or recent chat history. Call "
        "get_current_biometrics before answering questions about current "
        "biometric values unless current values have already been fetched for "
        "this question. Call get_procedure before answering questions about a "
        "specific procedure's steps, tasks, or details unless that full "
        "procedure text is already available. Call get_all_procedures when a "
        ...
        "open or display a procedure to resolve an error code, use the "
        "available procedure whose name ends with that exact bracketed code. "
