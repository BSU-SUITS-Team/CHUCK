...
async def put_user(username: str, db: Session = Depends(get_db)):
    ...
    crud.update_user_biometrics(db=db, user_id=user_id, new_biometrics=schemas.BiometricsBase())
    crud.update_user_location(db=db, user_id=user_id, new_location=schemas.LocationBase())
