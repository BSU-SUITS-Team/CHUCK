...
namespace Microsoft.MixedReality.OpenXR
{
    internal class MixedRealityFeatureValidator
    {
        internal static void GetValidationChecks(OpenXRFeature feature, List<ValidationRule> results, BuildTargetGroup targetGroup)
        {
            ...
            results.Add(new ValidationRule(feature)
            {
                message = "Run in Background is not recommended for HoloLens 2 applications.",
                error = false,
                checkPredicate = () => EditorUserBuildSettings.activeBuildTarget != BuildTarget.WSAPlayer || !PlayerSettings.runInBackground,
                fixIt = () => PlayerSettings.runInBackground = false
            });
        }
